...
Additionally, the simplification hides the [reuse of small `int` values](https://docs.python.org/3/c-api/long.html#c.PyLong_FromLong), an optimization that might otherwise confuse beginner Python programmers. For instance, after executing `a[1]+=1; b[1]+=1`, the value `201` is shared between `a` and `b`, whereas executing `a[2]+=1; b[2]+=1` does not result in sharing the value `301`.
