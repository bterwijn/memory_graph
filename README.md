...
Also note the difference between statement `b += [1]` that changes `b` (and `a`) and statement `c = b + [300]` that first create the new value `b + [300]` and assigns that value to `c` without effecting `b`. So here we see that `x += y` is not the same as `x = x + y` for values of mutable type.
