...
### Simplified Graph ###
By default, memory_graph simplifies the visualization (and the viewer's mental model) by **not** creating separate nodes for immutable types like `bool`, `int`, `float`, `complex`, and `str`. This simplification can sometimes be slightly misleading. As in the example below, after a shallow copy, lists `a` and `b` technically share their `int` values, but the graph makes it appear as though `a` and `b` each have their own copies. However, since `int` is immutable, this simplification will never lead to unexpected changes or bugs—changing `a` won’t affect `b`.
