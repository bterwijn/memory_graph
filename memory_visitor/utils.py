...
def front_back_split(iterable, front_size, back_size):
...
def front_back_repeat_split(iterable, front_size, back_size, block_size):
    if isinstance(iterable, types.GeneratorType):
        while True:
            block = tuple(itertools.islice(iterable, block_size))
            if not block:
                break
            yield front_back_split(block, front_size, back_size)
    else:
        index=0
